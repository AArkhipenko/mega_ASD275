...
public:
    ...
    // Сохранить новое значение по адресу (хранилище может обслуживать
    // несколько адресов); при изменении системного параметра вызвать
    // связанный parameter_translator.
...
| `axis_settings` | Настройки оси: скорость (VP 0x6000) и оборотов на градус (VP 0x6002) | системные переменные — одно хранилище |
| `slave_addr_storage` | Адрес/скорость Modbus-сети | системная переменная |

Системные переменные, которые задаются вместе и редко изменяются (настройки оси), объединяются в **одно хранилище**, принимающее несколько адресов (`owns()`, `address_at()`): например, `axis_settings` обслуживает VP 0x6000 (скорость) и VP 0x6002 (оборотов на градус). Транслятор кэширует настройки и обновляет кэш только по уведомлению об изменении хранилища — настройки не перечитываются и не изменяются при каждом движении.
...
Настройки оси (скорость и масштаб) транслятор берёт из **локального кэша**, который обновляется по уведомлению от хранилища настроек (`on_settings_changed`), а не при каждом перемещении.
...
| `0x6000` | Скорость привода | об/мин | `axis_settings` | sysvar → кэш транслятора |
| `0x6002` | Масштаб «градус → обороты» | оборотов на градус | `axis_settings` | sysvar → кэш транслятора |
